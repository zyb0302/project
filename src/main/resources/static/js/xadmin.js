;!function (win) {
	...
	//获取url传递的参数。编辑页面使用
    Xadmin.prototype.getQueryString = function(param) {
    ...
    //发送post请求，用于数据添加和编辑
    Xadmin.prototype.post = function(url,data) {
        $.ajax({
            type : "POST",
    ...
    //通过参数查询,并初始化编辑表单，用于页面编辑
    Xadmin.prototype.initTableData = function(url) {
        $.ajax({
            type : "POST",
            dataType: "json",//通过GET方式上传请求
            contentType : "application/json",//上传内容格式为json结构
    ...
    //根据id删除数据
    Xadmin.prototype.deleteData = function(url,id) {
        $.ajax({
            type : "get",
            async: false ,
            url : url,
    ...
    //批量删除数据
    Xadmin.prototype.batchDeleteData = function(url) {
        $.ajax({
            type : "get",
            async: false ,
            url :  url,
...
layui.use(['layer','element','jquery'],function() {
    ...
    layui.data('tab_list',null);  //清除tab缓存
    // 打开页面初始
    xadmin.init();
